        ...
        return {
            getFunctionCode(fn: Function) {
                ...
                return fnString
                    .replace(/^\(\)[ ]{0,}=>[ ]{0,}\{/, '')
                    .replace(/}$/, '')
                    .replace(/pagebuilder\.getValueByStateId\(['"][0-9a-zA-Z]+['"]\)/ig, (key: string) => { const keyCleared = key .replace(/pagebuilder\.getValueByStateId\(['"]/, '') .replace(/['"]\)$/, ''); return `state${keyCleared}` })
                    .replace(/pagebuilder\.setValueByStateId\(['"][0-9a-zA-Z]+['"][ ]{0,}\,[ ]{0,}['"][0-9a-zA-Z]+['"]\)/ig, (key: string) => { 
